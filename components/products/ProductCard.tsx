...
export const ProductCard:FC<Props> = ({product, categoryProduct}) => {
  ...
  return (
    <Link
      ...
      className='p-3 rounded-2xl w-full max-w-[350px] shadow-lg shadow-slate-400 dark:shadow-slate-800 transition-all flex flex-col group bg-white/50 md:hover:bg-gradient-to-br from-primary-blue from-10% to-primary-purple to-90% md:hover:text-white overflow-hidden group dark:bg-slate-900'
    >
      <div className="relative w-full flex items-center justify-center max-h-[250px] ">
        {images && 
          <img 
            src={images[0]}
            alt={serie}
            className='size-full object-contain group-hover:scale-105 transition min-h-[150px]' 
            loading="lazy"
          /> 
        }
      </div>
